...
@implementation AVIMTypedMessage (Model)
// 当 Model 转为 JSON 完成后，该方法会被调用。
// 你可以在这里对数据进行校验，如果校验不通过，可以返回 NO，则该 Model 会被忽略。
// 你也可以在这里做一些自动转换不能完成的工作。
- (BOOL)modelCustomTransformToDictionary:(NSMutableDictionary *)dic {
    dic[@"fileUrl"] = self.file.url;
    dic[@"fileId"] = self.file.objectId;
    dic[@"fileSize"] = [NSNumber numberWithInteger: self.file.size];
    dic[@"fileMimeType"] = self.file.mimeType;
    dic[@"fileName"] = self.file.name;
    dic[@"file"] = [self.file yy_modelToJSONObject];
